...
from app.schemas.combat import FightRequest, CombatResultResponse
from app.core.constants import STAMINA_COST, STAMINA_COST_BOSS
...
@router.post("/fight", response_model=CombatResultResponse)
async def fight(request: FightRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # locked query (FOR UPDATE) prevents two simultaneous fight requests from both passing the stamina check
    ...
    stamina_cost = STAMINA_COST_BOSS if zone_monster["is_zone_boss"] else STAMINA_COST
