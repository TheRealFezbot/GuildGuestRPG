...
async def get_zones(db: Session = Depends(get_db), character = Depends(get_current_character)):
    return get_zones_for_character(db, character.id)

@router.get("/monsters/{monster_id}", response_model=MonsterWithProgressResponse)
async def get_monster(monster_id: str, db: Session = Depends(get_db), character = Depends(get_current_character)):
...
async def get_monsters(zone_id: str, db: Session = Depends(get_db), character = Depends(get_current_character)):
